<script lang="ts">
  ...
  import {
    filterTimetableByDay,
    queryAvailableLessons,
  ...
  onMount(() => {
    chooseModStateCleanUpFunction = chooseModState.subscribe(
      async (chooseModState) => {
        lmao = {};
        for (let day = 0; day < max_days_displayed; day++) {
          let mods = await queryAvailableLessons(
            day,
            semester,
            acadYear,
            chooseModState,
